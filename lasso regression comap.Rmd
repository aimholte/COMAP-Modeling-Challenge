...
```{r}
library(plm)
consumption <- cleandata$RETCB
remove <- c("RETCB")
predictors <- cleandata[, !names(cleandata) %in% remove]
modlist <- list()
for(i in 1:10){
  for(j in 1:409){
    
    plm.mod.2 <- plm(RETCB ~ names(cleandata)[j], data = cleandata)
    adjr2 <- r.squared(plm.mod.2, dfcor = TRUE)
    modlist(j) <- plm.mod.2
  }
  maxR2 <- which.max(modlist)
  print(modlist(maxR2))
}
```
```{r}
